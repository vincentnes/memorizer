...
- Article title and tags fields in Memory Training.
- Save Article from the passage box.
- Article Library panel with search and status filter.
- Load saved articles back into Memory Training.
- Edit saved articles by loading them, changing title/tags/content, and saving again.
- Delete saved articles from the library.
- Add browsing visits to Article Library from Visit Details.
- Visit learning status in the Visits table: Not reviewed, Needs content, Added, Learning, Due, Mastered.
- Recall attempts linked to saved articles.
- Local JSON export/import for saved articles and recall sessions.

Remaining long-term data work:

- Migrate persistent data from localStorage to IndexedDB or SQLite when the app moves toward desktop packaging.
...
- Confidence rating added to the recall review step.
- Recall score and confidence calculate the next review date.
- Review intervals: 1 day, 3 days, 7 days, and 14 days.
- Article statuses now include New, Learning, Due, and Mastered.
- Due Today queue shows articles whose next review date has arrived.
- Memory Training navigation shows an overdue review count.
- Loading a due item starts another recall session from the saved article.

Still remaining for the full phase:

- Custom review intervals.
- More nuanced scheduling rules based on repeated attempts.
- Manual status overrides.
- Dedicated review history view by article.
- Notification/reminder support outside the app.
...
Recommended next sprint: finish Phase 2 polish, then move to Phase 3 recall modes.
